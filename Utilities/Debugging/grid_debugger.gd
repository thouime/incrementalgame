...
func _draw() -> void:
	...
	var ground_layer : TileMapLayer = world.get_node("Grass")
	draw_tilemaplayer_grid(ground_layer)

# Function to draw the grid for a specific TileMapLayer
func draw_tilemaplayer_grid(layer: TileMapLayer) -> void:
	# Access the TileSet from the TileMapLayer
	var tile_set : TileSet = layer.tile_set  
	if tile_set:
		# Get the size of the tiles from the TileSet
		var tile_size : Vector2i = tile_set.get_tile_size() 
		...
		var map_rect : Rect2 = layer.get_used_rect()
		var start_pos : Vector2 = map_rect.position
		var end_pos : Vector2 = map_rect.end

		# Position of the TileMapLayer in the parent world space
		var layer_position : Vector2 = layer.global_position
		# 0,0 of the TileMapLayer in world space
		var tilemap_origin_world_pos : Vector2 = layer_position
		...
		var start_pos_int := Vector2i(
			floor(start_pos.x / tile_size_f.x), 
			floor(start_pos.y / tile_size_f.y)
		)
		var end_pos_int := Vector2i(
			floor(end_pos.x / tile_size_f.x), 
			floor(end_pos.y / tile_size_f.y)
		)

		# Loop through all the tiles and draw grid lines
		# Draw vertical grid lines
		for x in range(start_pos_int.x, end_pos_int.x + 1):
			var start_pos_local : Vector2 = (
				layer.map_to_local(Vector2i(x, start_pos_int.y))
			)
			var end_pos_local : Vector2 = (
				layer.map_to_local(Vector2i(x, end_pos_int.y))
			)
			
			# Convert start_pos_local and end_pos_local to Vector2 
			# before subtracting the tile size
			start_pos_local = (
				Vector2(start_pos_local.x, start_pos_local.y) - tile_size_f / 2
			)
			end_pos_local = (
				Vector2(end_pos_local.x, end_pos_local.y) - tile_size_f / 2
			)
		...
		for y in range(start_pos_int.y, end_pos_int.y + 1):
			var start_pos_local : Vector2 = (
				layer.map_to_local(Vector2i(start_pos_int.x, y))
			)
			var end_pos_local : Vector2 = (
				layer.map_to_local(Vector2i(end_pos_int.x, y))
			)
